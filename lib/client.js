...
dav.Client.prototype = {
	...
	var baseParts = this.parseUrl(this.resolveUrl(url));
	var newUrl = "";
	if (/^https?:\/\//i.test(baseParts.url)) {
		newUrl =
			baseParts.root + "/dav_proxy/" + method + "/" + baseParts.path;
	} else {
		newUrl = "/dav_proxy/" + method + baseParts.url;
	}
